...
int main(int argc, char** argv) {    
    ...
    desc.add_options()
        ...
        ("dissipationModelParameterFolder", po::value<std::string>(),"(string) the folder where the parameters for the dissipation model are contained, each type can have a parameter file as a mapping of node->parameter(or parameters), if a file is missing for a type, than the parameters for that type will be 0, same can be said about nodes with no mapping. if not specified, the default parameters are used or the parameters in dissipationModelParameters parameter are used")
        ...
        ("conservationModelParameterFolder", po::value<std::string>(),"(string) the folder where the parameters for the conservation model are contained, each type can have a parameter file as a mapping of node->parameter(or parameters), if a file is missing for a type, than the parameters for that type will be 0, same can be said about nodes with no mapping. if not specified, the default parameters are used or the parameters in conservationModelParameters parameter are used")
