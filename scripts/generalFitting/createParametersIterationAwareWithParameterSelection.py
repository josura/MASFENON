...
Also has parameter selection to select the type of parameter to be updated (dissipation, propagation, conservation).
...
Inputs (per <type>.tsv):
  ...
  --parameter-selection : Type of parameter to update: dissipation, propagation, conservation
...
- Sensibility represents an approximation of the gradient of the error, although it needs to only be computed when changing one parameter at a time. So it represents a partial derivative more than a full derivative.
