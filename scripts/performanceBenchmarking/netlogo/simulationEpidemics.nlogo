...
globals [
  start-time        ;; for measuring running time
  max-steps         ;; maximum number of iterations
  log-file          ;; path to the shared output file
]
...
to setup-go
  ifelse ticks = 0 [
    clear-all
    set log-file "/home/josura/Projects/ccc/MASFENON/scripts/performanceBenchmarking/netlogo_performance.tsv"
    set max-steps 100
    reset-timer
  ...
  [
    if ticks >= max-steps [
      let elapsed-time timer - start-time
      log-results elapsed-time
      reset-ticks
      stop
    ]
...
to log-results [elapsed-time]
  file-open log-file
  file-print (word number-of-nodes "\t" ticks "\t" elapsed-time)
  file-close
end
@#$#@#$#@
GRAPHICS-WINDOW
