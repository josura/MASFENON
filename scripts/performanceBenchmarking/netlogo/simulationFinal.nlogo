...
to-report list-files [folder]
  let result shell:exec (word "ls " folder)
  let filenames string:split-on result "\n"
  if not empty? filenames and last filenames = "" [
    set filenames but-last filenames
  ]
  report map [ f -> word folder "/" f ] filenames
end
...
to setup
  ...
  let graph-files list-files graph-folder
